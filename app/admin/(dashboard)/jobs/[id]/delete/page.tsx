...
import { deleteJobPosting } from '@/lib/supabase/content-actions'
    ...
    <div className="max-w-md mx-auto space-y-6 pt-12">
      ...
      <div className="bg-card border border-destructive/30 rounded-2xl p-6 shadow-xl text-center space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-xl font-bold text-foreground">Delete Job Posting?</h1>
          <p className="text-sm text-muted-foreground">
            Are you sure you want to delete the job posting for <strong className="text-foreground">{job.title}</strong> ({job.location})? 
            This opening will be permanently deleted from the website careers board.
          </p>
        </div>

        <form action={handleDelete} className="flex gap-3">
          <Link href="/admin/jobs" className="flex-1 bg-muted hover:bg-muted text-foreground text-sm font-semibold py-2.5 rounded-xl border border-border transition-all text-center block">
            Cancel
          </Link>
          <button
            type="submit"
