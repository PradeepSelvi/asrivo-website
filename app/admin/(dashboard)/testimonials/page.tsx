...
import TestimonialsDndTable from './testimonials-dnd-table'
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage client reviews, ratings, and featured status. Drag to reorder.</p>
        </div>
        {isHigh && (
      ...
      {!isHigh && (
        <div className="border border-blue-900/40 bg-blue-950/30 rounded-xl px-5 py-4 text-sm text-blue-300 flex items-center gap-3">
          ...
          You have <strong>Editor</strong> access. You can edit, verify, and reorder testimonials but cannot create or delete them.
        </div>
      )}

      {error && <div className="p-6 text-destructive text-sm bg-card border border-border rounded-2xl">Error loading testimonials: {error.message}</div>}
      {!error && (!testimonials || testimonials.length === 0) ? (
        ...
        testimonials && <TestimonialsDndTable testimonials={testimonials} isHigh={isHigh} />
      )}
    </div>
  )
}
