...
import { Mail, Building, Clock, Eye } from 'lucide-react'
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">All contact form submissions from website visitors.</p>
      </div>

      {/* Status Stats Row */}
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-card border border-red-900/30 rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-destructive">{statusCounts.unread}</p>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-background/60 border-b border-border">
                <tr>
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Subject / Message</th>
              ...
              <tbody className="divide-y divide-border">
                {contacts?.map((contact) => (
                  <tr key={contact.id} className="hover:bg-muted/20 transition-colors cursor-pointer">
                    ...
                    <td className="px-6 py-4 max-w-xs">
                      {contact.subject && <p className="font-semibold text-foreground text-xs mb-0.5">{contact.subject}</p>}
                      <p className="text-xs text-muted-foreground line-clamp-2">{contact.message}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${
                        contact.status === 'unread' ? 'bg-red-500/10 text-destructive border border-red-500/20' :
